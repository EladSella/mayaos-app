...
export const LiveFeed: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      <div>
        <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] mb-6 pl-1">Happening Today</h3>
      ...
      <div>
        <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-6 pl-1">Recent Updates</h3>
      ...
      <button className="w-full py-2.5 text-xs font-semibold text-slate-500 hover:text-slate-800 border border-slate-200 hover:border-slate-300 rounded-xl transition-all mb-8 bg-white/50 backdrop-blur-sm tracking-wide uppercase hover:bg-white hover:shadow-sm">
        View All History
      </button>

      {/* Agency Logos */}
      <div className="flex items-end justify-center gap-8 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500 pt-8 border-t border-slate-200 mt-auto saturate-0 hover:saturate-100">
        <img src="/logos/neukleos.png" alt="Neukleos" className="h-8 object-contain" />
        <div className="h-6 w-px bg-slate-200"></div>
        <img src="/logos/image_and_time.png" alt="Image & Time" className="h-6 object-contain" />
      </div>
    </div>
  );
};
  ...
  return (
    <div className="flex gap-4 group">
      ...
      <div className="pb-3 border-b border-slate-100 last:border-0 w-full group-hover:pl-1 transition-all">
        ...
        <p className="text-xs text-slate-500 mt-0.5 leading-relaxed font-light">{event.body}</p>
